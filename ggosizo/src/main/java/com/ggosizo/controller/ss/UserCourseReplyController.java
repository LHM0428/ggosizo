...
import com.ggosizo.service.ss.UserCourseReplyService;

@RestController
@RequestMapping("/userCourseReplies")
public class UserCourseReplyController {

	@Inject
	private UserCourseReplyService service;
	...
	public ResponseEntity<List<Reply>> list(@PathVariable("no")int no){
		
		ResponseEntity<List<Reply>> entity = null;
		try {
			entity = new ResponseEntity<List<Reply>>(service.list(no), HttpStatus.OK);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public ResponseEntity<String> update(@PathVariable("reply_no") int reply_no,
			@RequestBody UserCourseReplyVO vo){
	...
	public ResponseEntity<Map<String, Object>> listPage(
		...
		try {
			...
			List<Reply> list = service.listReplyPage(no, cri);
