...
<c:if test="${pageMaker.prev}">
		<li><a href="javascript:myCourse('${u_id}',${(pageMaker.startPage-1)})"
		class="w3-hover-theme">&laquo;</a></li>
</c:if>
	<c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="idx">
		<li
			<c:out value="${pageMaker.cri.page == idx?'class =active':''}"/>>
			<a href="javascript:myCourse('${u_id}',${idx})" class="w3-hover-theme">${idx}</a>
		</li>
	</c:forEach>
<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
		<li><a href="javascript:myCourse('${u_id}',${(pageMaker.endPage+1)})"
