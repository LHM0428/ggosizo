		...
		/* 핸드폰이 숫자만 들어가는지 체크 */
			...
			$.ajax({
				...
				success : function(response) {
					alert('수정되었습니다.');
					window.location.replace('/mypage?infoModify=true');
				},
...
function myLike(u_id,page){
	$.ajax({
		url:'/mypage/myLikeList?u_id='+u_id+'&page='+page,
		method:'get',
		success:function(result){
			var resultArr = result.split("@paging");
			$('#likeTable').html(resultArr[0]);
			$('#likePage').html(resultArr[1]);
		}
	})
}

function myCourse(u_id,page){
	$.ajax({
		url:'/mypage/myCourseList?u_id='+u_id+'&page='+page,
		method:'get',
		success:function(result){
			var resultArr = result.split("@paging");
			$('#myCourseTable').html(resultArr[0]);
			$('#dcPage').html(resultArr[1]);
		}
	})
}
function myQnA(u_id,page){
	$.ajax({
		url:'/mypage/myQnAList?u_id='+u_id+'&page='+page,
		method:'get',
		success:function(result){
			var resultArr = result.split("@paging");
			$('#myQnATable').html(resultArr[0]);
			$('#QnAPage').html(resultArr[1]);
		}
	})
}